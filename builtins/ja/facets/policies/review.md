...
差分で共通ヘルパー、正規化関数、builder、adapter、状態遷移関数を追加・変更した場合、その抽象化が担う契約を明確にして既存の同種分岐へ照合する。

- その関数が統一する入力、出力、副作用、イベント、ログ、エラー種別、戻り値、後片付けを列挙する
- 同じ責務を持つ既存の return、throw、catch、early return、分岐、呼び出し元を検索する
- 既存分岐が新しい契約を満たさない場合は、コードが以前から存在していても変更の関係箇所として扱う
- 新しい抽象化を使う経路だけにテストが偏り、既存の同種分岐で契約が検証されていない場合はカバレッジ不足として扱う
